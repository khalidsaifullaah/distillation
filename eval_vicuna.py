...
def get_eval(input_tuple):
    sys_prompt, user_prompt, max_tokens, gpt_model_choice = input_tuple
    logging.basicConfig(level=logging.INFO)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gpt_model_choice", default="gpt-3.5-turbo")
    parser.add_argument(
        "--max-tokens",
        type=int,
        default=1024,
    ...
    for i in question_idx_list:
        ...
        prompts_list.append((sys_prompt, prompt, args.max_tokens, args.gpt_model_choice))


    with ThreadPoolExecutor(max_workers=3) as executor:
        reviews = list(executor.map(get_eval, prompts_list))
